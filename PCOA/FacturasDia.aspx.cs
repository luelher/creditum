...
{
	/// <summary>
	/// Descripción breve de FacturasDia.
	/// </summary>
	public class FacturasDia : System.Web.UI.Page
	{
		...
		private bool ClientesFacturar()
		{
			...
			try
			{
				...
				string strSQLQuery = "SELECT clientes_conf.ID_CLIENTE AS ID_CLIENTE,"
					...
					+ @"WHERE("
					+ "("
					+ @"(TO_DAYS(CURRENT_DATE()) - TO_DAYS(clientes_conf.FECHA_INI)) > 30)"
					+ ")"
					+ "GROUP BY clientes_conf.ID_CLIENTE "
					+ "HAVING "
					+ @"(((TO_DAYS(CURRENT_DATE()) - TO_DAYS(MAX(facturas.FECHA_HASTA)) > 30) AND (MONTH(MAX(facturas.FECHA_HASTA)) = MONTH(DATE_SUB(CURRENT_DATE(),INTERVAL 1 MONTH)))))"
					+ @" "
					+ "UNION"
					+ @" "
					+ "SELECT clientes_conf.ID_CLIENTE AS ID_CLIENTE,"
					+ @"CONCAT_WS('-',casas_comerciales.NOMBRE,clientes.NOMBRE) AS NOMBRE_CLIENTE,"
					+ "clientes.RIF AS RIF,"
					+ "clientes.NIT AS NIT,"
					+ "clientes.DIRECCION AS DIRECCION,"
					...
					+ @"WHERE"
					+ "("
					+ "((TO_DAYS(CURRENT_DATE()) - TO_DAYS(clientes_conf.FECHA_INI)) > 30) AND (facturas.ID_CLIENTE IS NULL)"
					+ @");";
				#endregion
		...
		private bool MostrarFacturas()
		{
			...
			try
			{
				...
				string strSQLQuery = "SELECT facturas.ID_FACTURA AS ID_FACTURA,"
					...
					+ @"LEFT JOIN casas_comerciales ON (clientes.ID_CASA_COMERCIAL=casas_comerciales.ID_CASA_COMERCIAL) "
					+ @"WHERE("
					+ @"(facturas.FECHA_HASTA = CURRENT_DATE())"
					+ @");";
				#endregion
